...
public class Algorithms {
    ...
    public static <T extends Comparable<T>> Graph<T> KruskalAlgorithm(Graph<T> graph){
        ...
        for (Edge<T> tEdge : edge) {
            if (result.get(tEdge.nameVertexFrom) == Integer.MAX_VALUE &&
                    result.get(tEdge.nameVertexTo) == Integer.MAX_VALUE) {
                result.put(tEdge.nameVertexFrom, countSet);
                result.put(tEdge.nameVertexTo, countSet);
                resultGraph.addEdge(tEdge.nameVertexFrom, tEdge.nameVertexTo, tEdge.weight);
                resultGraph.addEdge(tEdge.nameVertexTo, tEdge.nameVertexFrom, tEdge.weight);
                countSet++;
            } else {
                if (result.get(tEdge.nameVertexFrom) != Integer.MAX_VALUE &&
                        result.get(tEdge.nameVertexTo) == Integer.MAX_VALUE) {
                    result.put(tEdge.nameVertexTo, result.get(tEdge.nameVertexFrom));
                    resultGraph.addEdge(tEdge.nameVertexFrom, tEdge.nameVertexTo, tEdge.weight);
                    resultGraph.addEdge(tEdge.nameVertexTo, tEdge.nameVertexFrom, tEdge.weight);
                }
                if (result.get(tEdge.nameVertexFrom) == Integer.MAX_VALUE &&
                        result.get(tEdge.nameVertexTo) != Integer.MAX_VALUE) {
                    result.put(tEdge.nameVertexFrom, result.get(tEdge.nameVertexTo));
                    resultGraph.addEdge(tEdge.nameVertexFrom, tEdge.nameVertexTo, tEdge.weight);
                    resultGraph.addEdge(tEdge.nameVertexTo, tEdge.nameVertexFrom, tEdge.weight);
                }
                if (result.get(tEdge.nameVertexFrom) != Integer.MAX_VALUE &&
                        result.get(tEdge.nameVertexTo) != Integer.MAX_VALUE &&
                        result.get(tEdge.nameVertexFrom) != result.get(tEdge.nameVertexTo)) {
                    Set<T> keySet = result.keySet();
                    for (T vertex : keySet) {
                        if (result.get(vertex) != result.get(tEdge.nameVertexFrom) &&
                                result.get(vertex) != Integer.MAX_VALUE &&
                                result.get(vertex) == result.get(tEdge.nameVertexTo)) {
                            result.put(vertex, result.get(tEdge.nameVertexFrom));
                        }
                    }
                    resultGraph.addEdge(tEdge.nameVertexFrom, tEdge.nameVertexTo, tEdge.weight);
                    resultGraph.addEdge(tEdge.nameVertexTo, tEdge.nameVertexFrom, tEdge.weight);
    ...
    public static <T extends Comparable<T>> int getMin(List<Edge<T>> edges, Set<T> visited){
        int min = 0;
        int index = -1;
